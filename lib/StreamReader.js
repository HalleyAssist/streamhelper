const Q = require('@halleyassist/q-lite'),
      ...
      CloseOfStreamError = require('./CloseOfStreamError'),
      {Readable} = require('stream')

// DefaultTimeout of no timeout
...
class StreamReader extends Readable {
    constructor(s) {
        ...
        this._s.once('close', () => this._reject(new CloseOfStreamError()));
    }
    ...
    async _read(n){
        const buffer = await this.read(n)
        this.push(buffer)
    }

    readSync(size = undefined) {
    ...
    async _readUntil(buffer = null, fn = null, timeout = DefaultTimeout, cancelFn = ()=>{}){
        ...
        do {
            try {
                const bytesRead = await this.read(buffer, 0, 0, timeout, cancelFn);
                if(fn) {
                ...
                if(!fn && ex instanceof CloseOfStreamError) {
                    return ret
                }
                throw ex
            }
    ...
    async _peek(buffer, offset, length, readTimeout = DefaultTimeout, cancelFn = ()=>{}) {
        const bytesRead = await this.read(buffer, offset, length, readTimeout, cancelFn);
    ...
    read(buffer, offset = 0, length = undefined, readTimeout = DefaultTimeout) {
        ...
        ret = this._promiseRead(buffer, offset, length, readTimeout, c=>{ cancel = c })
        if(returnsBuffer) ret = ret.then(()=>buffer)
        ret.cancel = cancel
        return ret
    }

    async _promiseRead(buffer, offset, length, readTimeout = DefaultTimeout, cancelFn = ()=>{}) {
        ...
        do {
            ...
            if(readBuffer === null) {
                ...
                try  {
                    ...
                    if(ex instanceof CloseOfStreamError) {
                        throw new CloseOfStreamError()
                    }
                    throw ex
                }
            } else {
